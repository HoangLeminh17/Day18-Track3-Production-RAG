...
class BM25Search:
    ...
    def index(self, chunks: list[dict]) -> None:
        ...
        for chunk in chunks:
            text = chunk["text"]
            # Segment Vietnamese and tokenize
    ...
    def search(self, query: str, top_k: int = BM25_TOP_K) -> list[SearchResult]:
        ...
        # Tokenize query
        segmented = segment_vietnamese(query)
        tokenized_query = segmented.lower().split()
...
class DenseSearch:
    ...
    def index(self, chunks: list[dict], collection: str = COLLECTION_NAME) -> None:
        ...
        try:
            ...
            # Collection may not exist yet, try to delete first
            try:
...
def reciprocal_rank_fusion(results_list: list[list[SearchResult]], k: int = 60,
    ...
    # RRF scores dictionary: text → {"score": float, "result": SearchResult}
    rrf_scores = {}
    
    # Process each result list (from different search methods)
    ...
    results = []
    for text, item in sorted_items[:top_k]:
        result = item["result"]
        results.append(SearchResult(
            text=result.text,
            score=item["score"],
